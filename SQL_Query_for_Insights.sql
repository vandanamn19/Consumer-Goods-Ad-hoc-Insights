...
WITH cte1 AS (  SELECT   segment, Count(DISTINCT(product_code)) AS 2020_cnt  FROM     dim_product  JOIN     fact_sales_monthly using    (product_code)  WHERE    fiscal_year = 2020  GROUP BY segment), 
cte2 AS (  SELECT   segment, Count(DISTINCT(product_code)) AS 2021_cnt  FROM     dim_product  JOIN     fact_sales_monthly using    (product_code)  WHERE    fiscal_year = 2021  GROUP BY segment)SELECT segment,       2021_cnt AS product_count_2021,       2020_cnt AS product_count_2020,       2021_cnt-2020_cnt AS differenceFROM   cte1JOIN   cte2 using  (segment);
...
with cte1 as (
select channel,
round(sum(gross_price*sold_quantity)/100000,2) as total_gross_price_mln
from fact_sales_monthly as fsm join fact_gross_price
using(product_code)  
join dim_customer as c on 
c.customer_code=fsm.customer_code
...
select division,product_code,total_sold_quantity,drank from cte2
where drank<=3
